...
import { SERVER_API_URL } from "../../constants/api.js";
...
export default function CreateTab() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_API_URL}/books`, {
        method: "POST",
        headers: {
  ...
  const renderRatingPicker = () => {
    const stars = []; // Fixed typo from 'starts' to 'stars'

    for (let i = 1; i <= 5; i++) {
      stars.push(
        // Added missing closing parenthesis after setRating(i)}
        <TouchableOpacity
          key={i}
          onPress={() => {
            setRating(i);
    ...
    return stars; // Added missing return statement
  };
      ...
      <ScrollView>
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>Add fav Book</Text>
